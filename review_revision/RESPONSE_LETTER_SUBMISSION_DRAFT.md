...
We agree. The revised manuscript now reports same-workflow held-out test-set errors and a direct comparison with the unfine-tuned MACE-MPA-0 foundation model. Fine-tuning reduces the all-family held-out force RMSE from 285.2 to 20.1 meV A^-1. The all-family energy RMSE remains 39.1 meV atom^-1, mainly due to a systematic Si4--graphene family offset, and this limitation is now discussed explicitly. The text no longer treats this split as an independent transferability benchmark. A grouped split with foundation-assisted E0 estimation has also been run; its small grouped test split has force RMSEs of 19.6, 10.9, 13.7, 42.9, and 9.2 meV A^-1 for pristine graphene, monovacancy graphene, divacancy graphene, Stone--Wales graphene, and the Si4--graphene motif, respectively. Because this grouped test set has only five held-out frames and high-displacement snapshot errors remain mixed, we still present it as a diagnostic audit rather than as final force-field validation.
