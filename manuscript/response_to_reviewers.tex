...
\noindent\textbf{Revised title:} Machine-learning interatomic potentials for
battery materials: Defect-dependent lithium adsorption and local transferability
on graphene
...
with PBE-D3(BJ) adsorption energies across three preselected Li placements
per family and direct DFT force tests on displaced configurations. The latter
include an independently defined, post-training benchmark balanced across all five families
and a separate high-displacement comparison. Detailed split analyses, $E_0$
estimation, fixed-path scans, trajectory traces, and snapshot records are
retained in Methods or Supporting Information, where they support the science
without becoming the narrative.

The main manuscript also includes a dedicated Supporting Information section
(p.~11, lines 328--334) that summarizes the accompanying dataset analyses,
adsorption tables, force benchmarks, path scans, trajectory traces, and
converged DFT snapshot records.
...
Direct DFT calculations instead delimit the supported adsorption and
force-transferability conclusions and identify targeted future labels.

\changes Introduction, paragraph 3, p.~2, lines 46--53; new references 14 and
15, p.~13, lines 387--391. A six-row comparison of model, exploration, selection signal,
...
\changes New title and Abstract (one paragraph), p.~1, lines 1--27;
Introduction, paragraphs 3--5, pp.~2--3, lines 46--68.
...
Membership of the new five-family perturbation benchmark was also fixed without
MACE energies, forces, or committee disagreement. We now state these points
directly and avoid calling the committee an uncertainty ensemble.

\changes MACE potential development and validation, paragraph 4, p.~6, lines
164--172. Seed-resolved metrics remain in Supporting Information Section~1.
...
population-level transferability estimate. The revised text treats this split as a
correlation diagnostic and retains the original correlated-split metric only in the main
text and Supporting Information as an interpolation baseline. We added a
post-training force benchmark containing five predefined perturbations from
each of the five structural families (25 new DFT configurations). Configuration
selection used neither MACE predictions nor committee disagreement. This
balanced test complements, rather than replaces, the more severe nine-frame
monovacancy/Si$_4$ challenge set. All 25 single points reached the final
electronic-convergence criterion. Fine-tuning reduces the pooled force RMSE from
450.0 to 267.5 meV~\AA$^{-1}$ overall and lowers the family-resolved RMSE in
each of the five environments by 35.4--43.2\%. The frame-level force RMSE is
also lower for 23 of the 25 predefined configurations.

\changes MACE potential development and validation, paragraphs 1 and 5,
pp.~5--6, lines 132--142 and 173--184; new balanced-force results in Figure~2
(p.~8) and Results and Discussion, Section~3.3, paragraphs 1--4, pp.~9--10,
lines 250--275; family-resolved values in Supporting Information Table~4 (p.~3);
frame-level values are provided in the Supplementary Data archive. The correlated-split
285.2-to-20.1 meV~\AA$^{-1}$ metric has been removed from the Abstract.
...
curated Supplementary Data CSV columns from the historical
\texttt{barrier\_from\_*} names to \texttt{max\_minus\_start\_ev} and
\texttt{path\_span\_ev}.

\changes DFT labeling, paragraphs 3--4, pp.~4--5, lines 114--129; Scope and
limitations, paragraph 1, p.~11, lines 305--310; Supporting Information
Figure~3 and Table~5 (p.~5); curated file
...
generate displaced configurations, and their physical reliability is assessed
through direct DFT force comparisons rather than trajectory stability alone.
The manuscript does not report a diffusion coefficient.

\changes Finite-temperature snapshot generation, paragraph 1, pp.~6--7, lines
185--205, especially p.~6, lines 191--193. The trajectory traces and their seed dependence are
now in Supporting Information Figure~4 and Table~6 (p.~6).
...
\response We agree. Direct DFT force comparisons are now the central evidence in
Figure~2 and the Results. The new family-balanced test evaluates predefined
controlled local perturbations, while the original challenge set probes selected
high-displacement monovacancy and Si$_4$ configurations. Interpretation is tied
to measured force errors and, for the challenge set, to the specific regime of
short reconstructed C--C contacts. We do not infer that all high-displacement
configurations are out of domain. The residual balanced-test family errors of
246--297 meV~\AA$^{-1}$ are reported explicitly, so the new result supports
broader local improvement without being presented as global transferability.
On the strict nine-frame challenge, fine-tuning lowers the Si$_4$ force RMSE
from 310 to 113 meV~\AA$^{-1}$ but changes the monovacancy value only from
1133 to 1097 meV~\AA$^{-1}$, directly establishing the environment-resolved
contrast.

\changes Abstract (one paragraph), p.~1, lines 19--27; Figure~2, p.~8; Results
and Discussion, Sections~3.3--3.4, paragraphs 1--7, pp.~9--10, lines 250--299;
Conclusion, paragraphs 1--2, p.~11, lines 311--327.
...
\changes Finite-temperature snapshot generation, paragraph 1, p.~7, lines
200--205; Results and Discussion, Section~3.4, paragraph 3, p.~10, lines
291--299.
...
errors, and closes with the implication for targeted data expansion. The
correlated-split metric and detailed split, checkpoint, and snapshot
information have been moved to the main text or Supporting Information.

\changes Revised Abstract (one paragraph), p.~1, lines 9--27. Detailed split, $E_0$, path,
...
figures: a high-resolution structure overview and a four-panel vector summary
of multi-site Li adsorption, the balanced five-family DFT force benchmark, its
displacement-amplitude dependence, and the high-displacement force challenge.
The former fixed-path
...
\changes Main Figures~1--2 (pp.~3 and 8); Supporting Information Figures~2--4
(pp.~4--6), especially the revised path panels on p.~5 and MSD panels on p.~6.
...
have been removed. Operational logs, status language, and repeated gate lists
no longer appear in the scientific narrative. Limitations are
consolidated in one subsection rather than repeated as debug commentary.

\changes Title and Abstract (one paragraph), p.~1, lines 1--27; Introduction,
paragraphs 1--5, pp.~2--3, lines 30--68; Results and Discussion,
Sections~3.2--3.5, paragraphs 1--10, pp.~9--11, lines 236--310; Conclusion,
paragraphs 1--2, p.~11, lines 311--327.
...
\response We added a dedicated ``MACE potential development and validation''
subsection. It records the inherited architecture, cutoff,
...
metrics, seed sensitivity, $E_0$ details, and a workflow-comparison table. We
also added the complete selection and DFT protocol for the post-training,
independently defined 25-configuration benchmark.

\changes DFT labeling, paragraph 4, pp.~4--5, lines 121--131; MACE potential
development and validation, paragraphs 1--5, pp.~5--6, lines 132--184;
finite-temperature snapshot generation, paragraph 1, pp.~6--7, lines 185--205;
Supporting Information Tables~1--2 (p.~2).
