...
with PBE-D3(BJ) adsorption-energy checks across three prespecified Li placements
per family and direct DFT force tests on displaced configurations. The latter
include a model-blind, post-training benchmark balanced across all five families
and a separate high-displacement challenge set. Dataset-split audits, $E_0$
details, fixed-path scans, trajectory traces, and snapshot traceability are
...
\changes Introduction, p.~2, lines 46--54; new references 14 and 15, p.~13,
lines 390--394. A six-row comparison of model, exploration, selection signal,
...
\changes Potential development and validation workflow, p.~6, lines
173--193. Seed-resolved metrics remain in Supporting Information Section~1.
...
leakage audit and retains the original same-workflow metric only in the main
text and Supporting Information as an interpolation baseline. We added a
post-training force benchmark containing five prespecified perturbations from
...
also lower for 23 of the 25 prespecified configurations.

\changes Group-aware split and its limitation, p.~5, lines 135--145; benchmark
selection and DFT protocol, p.~6, lines 180--193; new balanced-force results in
Figure~2 (p.~8) and Results and Discussion, pp.~9--10, lines 260--300; detailed
frame-level values in Supporting Information Table~4 (p.~3). The same-workflow
...
\changes Methods, p.~5, lines 122--130; Scope and limitations, p.~11, lines
310--320; Supporting Information Figure~3 and Table~5 (p.~5); curated file
...
\changes Finite-temperature snapshot generation, p.~7, lines 194--215,
especially lines 200--201. The trajectory traces and their seed dependence are
...
Figure~2 and the Results. The new family-balanced test evaluates prespecified
...
environment-resolved DFT results, pp.~9--10, lines 260--300; Conclusion,
p.~11, lines 321--337.
...
\changes Methods, p.~7, lines 202--215; Results, pp.~10--11, lines 301--309.
...
correlated same-workflow split metric and detailed split, checkpoint, job, and
snapshot counts have been moved to the main text or Supporting Information.
...
30--69; Results and Discussion, pp.~7--11, lines 217--320; Conclusion, p.~11,
lines 321--337.
...
model-blind 25-configuration benchmark.

\changes Dataset scope, p.~5, lines 122--134; group-aware split, p.~5, lines
135--145; architecture and training protocol, pp.~5--6, lines 146--179;
balanced-test protocol, p.~6, lines 180--193; snapshot validation protocol,
p.~7, lines 194--215; Supporting Information Tables~1--2 (p.~2).
